...
if __name__ == '__main__':
    ...
    ports, useAccel, useLight, useADC, useWeather, ShimmerIDs[0], ShimmerIDs[1], ShimmerIDs[2], PLOT, numRelayStat, fileLengthSec, fileLengthDay, DeploymentID, networkNum = streamParseConfig()
    ...
    try:
        for i in range(numRelayStat):
            ...
            streamingProcs.append(multiprocessing.Process(target = stream.stream_process, args=(comm_queue, ports[i], useAccel[i], useLight[i], useADC[i], useWeather[i], ShimmerIDs, PLOT, fileLengthSec, fileLengthDay, DeploymentID, networkNum, i, lock, host)))
    except:
